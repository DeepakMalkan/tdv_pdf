...
class PdfBlockGenerator ():
    ...
    def generate_blocks_for_pages (self, indoc, basepath, pages, expected_factor):
        deal_page_list = []
        ...
        for page_index in pages:
            ...
            num_blocks = len (deal_page.block_list)
            if num_blocks == 0: # If no blocks were generated by looking for rectangles
                deal_page.generate_blocks_from_path (doc_page)  # then try to generate blocks from paths
                num_blocks = len (deal_page.block_list)
                if num_blocks == 0:
            ...
            if len (deal_page.missing_blocks_list) != 0:
                print (f"MISSING blocks for page {basepath}:{page_index}, expected_factor {expected_factor}, got {num_blocks}")
    ...
    def generate_and_draw_blocks_for_pages (self, inpath, basepath, pages, outpath, expected_factor):
        ...
        deal_page_list = self.generate_blocks_for_pages (indoc, basepath, pages, expected_factor)
    ...
    def generate_blocks_for_deal_section (self, inpath, deal_section, expected_factor):
        ...
        indoc = fitz.open (inpath)
        self.generate_blocks_for_pages (indoc, basepath, deal_section.page_list, expected_factor)
        indoc.close ()
